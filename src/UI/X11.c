...
static SELFMAKE_RESULT selfmake_connectToDisplay()
{
SELFMAKE_BEGIN()

    Data.Display_p = XOpenDisplay(NULL);
    SELFMAKE_CHECK_NULL(Data.Display_p)
...
static SELFMAKE_RESULT selfmake_createSimpleWindow(
...
static void selfmake_createGraphicsContext()
{
...
static void selfmake_createFont()
{
...
SELFMAKE_BEGIN()
    ...
    SELFMAKE_CHECK(selfmake_connectToDisplay())
...
// create window
    selfmake_createSimpleWindow(width, height);
    selfmake_createGraphicsContext();
    selfmake_createFont();
...
SELFMAKE_DIAGNOSTIC_END(SELFMAKE_ERROR_BAD_STATE)
}
...
SELFMAKE_BEGIN()
    ...
    while (XPending(Data.Display_p) > 0) 
    {
        ...
        {   
                 if (Event.xclient.data.l[0] == Data.DeleteAtom) {selfmake_handleGUIExit();}
        ...
        else if (Event.type == SelectionNotify)
        {
            ...
            SELFMAKE_CHECK(selfmake_handleGUIExpose())
        }
    }
...
SELFMAKE_BEGIN()
    ...
    if (count != 0) {selfmake_X11_drawTextLine(x, y + descent, text_p + count);}
