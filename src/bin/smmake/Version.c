    ...
    int version_p[3], long versionDates_pp[3][3])
{
    ...
    version_p[0] = getVersion(data_p, major_p);
    version_p[1] = getVersion(data_p, minor_p);
    version_p[2] = getVersion(data_p, patch_p);

    getVersionDate(data_p, major_p, versionDates_pp[0]);
    getVersionDate(data_p, minor_p, versionDates_pp[1]);
    getVersionDate(data_p, patch_p, versionDates_pp[2]);
    ...
    sm_Runtime *Runtime_p, SM_BYTE *version_p)
{
    ...
    if (Stage.length) {
        ...
        sprintf(version_p, "v%d.%d.%d %s", versionNums_p[0], versionNums_p[1], versionNums_p[2], stage_p);
    }
    else {
        sprintf(version_p, "v%d.%d.%d", versionNums_p[0], versionNums_p[1], versionNums_p[2]);
    }
}
