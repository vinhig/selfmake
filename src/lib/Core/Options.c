            ...
            &&  Parser_p->Definitions_p[d].Option.arguments == args) {
                ...
                for (int i = 0; i < args; ++i) {
                    ...
                    if (!strcmp(Parser_p->Definitions_p[d].Option.name_p, "b")) {
                        argument_p = sm_offsetBuildArgumentPrefix(Runtime_p, Parser_p, argument_p);
                    }
                    if (strcmp(argument_p, argv_pp[i])) {
...
SM_BEGIN()
    ...
    sm_operationf(Option_p->longOption ? "--%s %s" : "-%s %s", Option_p->name_p, Option_p->description_p);
